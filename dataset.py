...
class Batch_Balanced_Dataset(object):
    ...
    def get_batch(self):
        ...
        # print(len(self.dataloader_iter_list))
    ...
    #             print(f"⚠️ Unexpected error in dataloader {i}: {e}")
    #             continue

    #         balanced_batch_images.append(image)
    #         if isinstance(text, list):
    #             balanced_batch_texts.extend(text)
    #         else:
    #             balanced_batch_texts.append(text)

    #     balanced_batch_images = torch.cat(balanced_batch_images, dim=0)
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            try:
                ...
                print(f'1Corrupted image for {index}')
...
class RandomChannelOrder:
    def __call__(self, img: Image.Image) -> Image.Image:
        ...
        order = random.choice(perms)
        return Image.merge("RGB", order)
...
class DataAugment(object):
    ...
    def rand_aug(self, img):
        ...
        # print(self.augs, self.opt.augs_num,augs )
        for aug in augs:
